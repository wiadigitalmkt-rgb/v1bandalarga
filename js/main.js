...
/* ---------------------------------------------------------
   Contador do hero (roda uma vez, ao carregar)
--------------------------------------------------------- */
...
async function checkCoverage(bairroRaw) {
  ...
  // fallback local
  const found = FALLBACK_BAIRROS.find((b) => normalize(b.bairro).includes(term) || term.includes(normalize(b.bairro)));
  return found || null;
}
...
coverageForm.addEventListener("submit", async (e) => {
  ...
  if (result.status === "disponivel") {
    renderResult("ok", `Boa notícia! A V1 já atende o bairro <strong>${result.bairro}</strong>. <a href="${whatsUrl}" target="_blank" rel="noopener" style="text-decoration:underline">Peça sua instalação</a>.`);
  } else if (result.status === "em_expansao") {
    renderResult("warn", `Estamos expandindo a rede no bairro <strong>${result.bairro}</strong>. <a href="${whatsUrl}" target="_blank" rel="noopener" style="text-decoration:underline">Fale com a gente</a> pra saber a previsão.`);
  } else {
    renderResult("unknown", `Encontramos o bairro <strong>${result.bairro}</strong>, mas precisamos confirmar a cobertura. <a href="${whatsUrl}" target="_blank" rel="noopener" style="text-decoration:underline">Fale com a gente no WhatsApp</a>.`);
  }
});
